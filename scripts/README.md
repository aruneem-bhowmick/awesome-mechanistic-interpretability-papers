...
Pulls recent arXiv papers into `PAPER_INBOX.md` for manual triage. Curation stays
...
New candidates are appended to `PAPER_INBOX.md` under the matching section header,
...
`PAPER_INBOX.md` lives here. This is what makes rejection permanent: delete an entry
from `PAPER_INBOX.md` and it will not resurface on a later run, because the id stays
...
`peter-evans/create-pull-request` if `PAPER_INBOX.md` changed) is deliberately
